...
#
#	$Id: dods.pm 558 2008-09-03 18:37:25Z lifo $
#
package PS::Game::source::dods;
...
#		print scalar @$plrs, " $team members scored $val->{score} on map " . $m->name . "\n";
	...
	} elsif ($trigger eq 'dods_capture_area') {
	} elsif ($trigger eq 'captured_loc') {		# is now the main way for dods to record flag captures (plr trigger isn't used anymore)
		...
		if (@$plrs) {
			...
			if (@$list) {
				# Tracked under 'dods_control_point' to be
				# consistant in how it used to be tracked in the
				# original source::dods.
				$self->plrbonus('dods_control_point', 'enactor', $list);
			}
			$m->{mod}{'flagscaptured'}++;
...
sub event_plrtrigger {
	...
	} elsif ($trigger eq 'dods_object') {					# got TNT (pre source)
# these are useless, from the old days of the original DOD
#		@vars = qw( tnt );
	} elsif ($trigger eq 'dods_object_goal') {				# used TNT (pre source)
#		@vars = qw( tntused );

	} elsif ($trigger eq 'dods_control_point') {				# plr captured a flag
		@vars = ( $p1->{team} . 'flagscaptured', 'flagscaptured' );

	} elsif ($trigger eq 'dods_capture_area') {				# plr captured an area (pre source)
		@vars = ( $p1->{team} . 'flagscaptured', 'flagscaptured' );
...
###		@vars = ( $p1->{team} . 'areascaptured', 'areascaptured' );
	...
	} elsif ($trigger eq 'dods_blocked_point') {
...
sub event_dods_teamscore {
	...
	if (!$self->{dods_teamscore}) {
		$self->{dods_teamscore} = { team => $team, score => $score, numplrs => $numplrs };
		return;
	}
...
#	print "axis   = " . scalar(@{$self->get_team('axis', 1)}) . "\n";
	...
	if ($score > $self->{dods_teamscore}{score}) {
		$teamwon  = $team;
		$teamlost = $team eq 'axis' ? 'allies' : 'axis';
		$won  = $teams->{ $teamwon };
		$lost = $teams->{ $teamlost };
	} elsif ($self->{dods_teamscore}{score} > $score) {
		$teamwon  = $self->{dods_teamscore}{team};
		$teamlost = $self->{dods_teamscore}{team} eq 'axis' ? 'allies' : 'axis';
		$won  = $teams->{ $teamwon };
		$lost = $teams->{ $teamlost };
	} else {
		# do mot count 'draws'
